...
pub struct Ipv4EndpointSlice<'a> {
    slice: &'a [u8],
}

impl<'a> Ipv4EndpointSlice<'a> {
...
impl<'a> From<Ipv4EndpointSlice<'a>> for super::Ipv4EndpointOption {
...
mod test {
    use super::*;
    use crate::sd::options::{Ipv4EndpointOption, TransportProtocol};

    #[test]
    fn from_slice() {
        assert!(Ipv4EndpointSlice::from_slice(&[]).is_err());
        assert!(Ipv4EndpointSlice::from_slice(&[0u8; 8]).is_err());
        let err = Ipv4EndpointSlice::from_slice(&[]).unwrap_err();
        assert_eq!(err.required_len, Ipv4EndpointSlice::LEN);
        assert_eq!(err.len, 0);
        assert_eq!(err.len_source, LenSource::Slice);
        assert_eq!(err.layer, Layer::SdOption);

        let s = Ipv4EndpointSlice::from_slice(&[0u8; 9]).unwrap();
        assert_eq!(s.slice(), &[0u8; 9]);

        assert!(Ipv4EndpointSlice::from_slice(&[0u8; 12]).is_err());
    }

    #[test]
    fn accessors() {
        let slice = [0x00, 0xc0, 0xa8, 0x01, 0x01, 0x00, 0x06, 0x1f, 0x90];
        let s = Ipv4EndpointSlice::from_slice(&slice).unwrap();
        ...
        let s = Ipv4EndpointSlice::from_slice(&slice_udp).unwrap();
        ...
        let s = Ipv4EndpointSlice::from_slice(&slice_generic).unwrap();
    ...
    fn from_conversion() {
        let slice = [0x00, 0xc0, 0xa8, 0x01, 0x01, 0x00, 0x06, 0x1f, 0x90];
        let s = Ipv4EndpointSlice::from_slice(&slice).unwrap();
        let opt = Ipv4EndpointOption::from(s);
