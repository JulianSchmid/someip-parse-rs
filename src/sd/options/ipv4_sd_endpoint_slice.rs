...
pub struct Ipv4SdEndpointSlice<'a> {
    slice: &'a [u8],
}

impl<'a> Ipv4SdEndpointSlice<'a> {
...
impl<'a> From<Ipv4SdEndpointSlice<'a>> for super::Ipv4SdEndpointOption {
...
mod test {
    use super::*;
    use crate::sd::options::{Ipv4SdEndpointOption, TransportProtocol};

    #[test]
    fn from_slice() {
        assert!(Ipv4SdEndpointSlice::from_slice(&[]).is_err());
        assert!(Ipv4SdEndpointSlice::from_slice(&[0u8; 8]).is_err());
        let err = Ipv4SdEndpointSlice::from_slice(&[]).unwrap_err();
        assert_eq!(err.required_len, Ipv4SdEndpointSlice::LEN);
        assert_eq!(err.len, 0);
        assert_eq!(err.len_source, LenSource::Slice);
        assert_eq!(err.layer, Layer::SdOption);

        let s = Ipv4SdEndpointSlice::from_slice(&[0u8; 9]).unwrap();
        assert_eq!(s.slice(), &[0u8; 9]);

        let longer = [0u8; 12];
        let s = Ipv4SdEndpointSlice::from_slice(&longer).unwrap();
